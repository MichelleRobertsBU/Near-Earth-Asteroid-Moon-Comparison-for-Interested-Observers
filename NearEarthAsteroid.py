...
##define figure and axes
fig, ax = plt.subplots()
plt.suptitle('Comparison to the Moon distance of the ten closest Asteroids.', fontsize=15)
...
#Dataframe of Near Earth Asteroids Minimum Distance in comparison to Moon's Distance from Earth.
##Requirement (2) Manipulate and clean data.
df2 = pd.read_csv("NEO_Earth_Close_Approaches.csv", usecols=["Object","CA DistanceMinimum (km)"])
df2.head()
df2['CA DistanceMinimum (km)']= df2['CA DistanceMinimum (km)'].div(384,400)
df2 = pd.DataFrame(df2.tail(10))

## Create table
table = ax.table(cellText=df2.values, colLabels=('Object', 'Lunar Distance'), colColours =["palegreen"] * 10, cellLoc ='center', loc='center')
table.set_fontsize(12)
table.scale(1,2)
...
ax1.axis('equal')
plt.pie(df2['CA DistanceMinimum (km)'], labels = df['Object'],textprops={"fontsize":10})
plt.show()
...
#to ultimately saving the planet from future disasters.
